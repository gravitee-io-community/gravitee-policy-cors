 ...
 */
public class CorsPolicyHeaderAccessControlExposeHeaders extends CorsPolicyHeader {
    ...
    public void testHeaderWhenConfigurationDeactivatedAndExistingInResponse() throws Exception {
        Map<String, String> headers = new HashMap<String, String>() {
            {
                put(HttpHeader.ACCESS_CONTROL_EXPOSE_HEADERS.toString(), DEFAULT_HEADER_VALUE);
            }
        };
        ...
        assertEquals(DEFAULT_HEADER_VALUE, headers.get(HttpHeader.ACCESS_CONTROL_EXPOSE_HEADERS.toString()));
    ...
    public void testHeaderWhenConfigurationActivatedAndNotOverridingAndExistingInResponse() throws Exception {
        ...
        configuration.getAccessControlExposeHeaders().setEnabled(true);
        cors.onResponse(request, response, policyChain);
    ...
    public void testHeaderWhenConfigurationActivatedAndNotOverridingWithNullValueAndNotExistingInResponse() throws Exception {
        ...
        configuration.getAccessControlExposeHeaders().setEnabled(true);
        configuration.getAccessControlExposeHeaders().setValue(null);
        cors.onResponse(request, response, policyChain);
    ...
    public void testHeaderWhenConfigurationActivatedAndNotOverridingWithNonNullValueAndNotExistingInResponse() throws Exception {
        ...
        configuration.getAccessControlExposeHeaders().setEnabled(true);
        configuration.getAccessControlExposeHeaders().setValue("X-Foo-Header, X-Bar-Header");
        cors.onResponse(request, response, policyChain);

        assertEquals(1, headers.size());
        assertEquals("X-Foo-Header, X-Bar-Header", headers.get(HttpHeader.ACCESS_CONTROL_EXPOSE_HEADERS.toString()));
        verify(policyChain).doNext(request, response);
    }
    ...
    public void testHeaderWhenConfigurationActivatedAndOverridingWithNullValueAndNotExistingInResponse() throws Exception {
        ...
        configuration.getAccessControlExposeHeaders().setEnabled(true);
        configuration.getAccessControlExposeHeaders().setOverridden(true);
        configuration.getAccessControlExposeHeaders().setValue(null);
        cors.onResponse(request, response, policyChain);
